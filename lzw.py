...
def extAsciiCompress(chaine):
	...
	La fonction devrait nous renvoyer : B{['T', 'O', 'B', 'E', 'O', 'R', 'N', 'O', 'T', 256, 258, 260, 265, 259, 261, 263]}
...
def extAsciiUncompress(chaine):
	...
	I{chaine = ['T', 'O', 'B', 'E', 'O', 'R', 'N', 'O', 'T', 256, 258, 260, 265, 259, 261, 263]
...
def extAsciiCompFile(fileIn, fileOut):
	"""
	La fonction compFile utilise la table ASCII étendue pour compresser. Cette technique
	se base sur 16 bits pour comprésser le fichier d'entrée I{fileIn}. Le fait d'utiliser
	la table ASCII étendue fait qu'on perd un peu en performance (la méthode demandée est 
	sur 9 bits), par contre, elle est bien plus simple à implémenter. 
	
	@param fileIn: Fichier à compresser
	@param fileOut: Fichier de sortie, dans lequel sera mis le fichier compréssé.

	@type fileIn: C{list}
	@type fileOut: C{str}

	@return: C'est une procédure, il n'y a donc pas de valeur retournée. Cette procédure
	ne sert qu'à comprésser et rediriger le flux vers un fichier de sortie.
	"""
	filename = fileIn
...
def extAsciiDecompFile(fileIn, fileOut):
	"""
	La fonction decompFile utilise la table ASCII étendue pour décompresser. Cette technique
	se base sur 16 bits pour décomprésser le fichier d'entrée I{fileIn}. Le fait d'utiliser
	la table ASCII étendue fait qu'on perd un peu en performance (la méthode demandée est 
	sur 9 bits), par contre, elle est bien plus simple à implémenter. 
	
	@param fileIn: Fichier à décompresser
	@param fileOut: Fichier de sortie, dans lequel sera mis le fichier décompréssé.
...
if __name__ == '__main__':
	...
	while flag:
		try:
			choice = int(input("LZW compressor / uncompressor\nChoose an action to perform : \n[1] - Compress\n[2] - Uncompress\n> "))
			flag = False
		except ValueError:
		...
		if flag == False:	
			if choice == 1:
				...
				tauxComp = (extAscii2*100)/extAscii1
				...
				tauxComp2 = (bin2*100)/bin1

				print("Extend ASCII method.")
				print("Le taux de compression est de : {0} %".format(round(tauxComp,2)))
				print("Le fichier passe de : " + str(os.stat(fileToCompress).st_size) + " octets à : " + str(os.stat(ASCIIDecomp).st_size) + " octets")
				print("La compression s'est faite en : {0} secondes\n".format(round(end_timeASCII, 4)))

				print("Binary method.")
				print("Le taux de compression est de : {0} %".format(round(tauxComp2,2)))
				print("Le fichier passe de : " + str(os.stat(fileToCompress).st_size) + " octets à : " + str(os.stat(binaryDecomp).st_size) + " octets")
				print("La compression s'est faite en : {0} secondes".format(round(end_timeBinary, 4)))
				...
				print("Binary method.")
				print("Le fichier {0} est décompréssé dans {1}".format(binaryDecomp, output2))
				flag = False
			else:
